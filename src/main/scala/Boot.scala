...
class UserGrpcService extends UserGrpcServiceLike {

  override def getUsers(
    req: GetUsersReqProto
  ): Future[GetUsersRespProto] = ???
}

object FilterModeValidator {
  def validate(filterModeProto: FilterModeProto): Option[FilterMode] = PartialFunction.condOpt(filterModeProto) {
...
object FilterValidator {
  ...
  def validateAge(filter: Option[AgeFilterProto]): Option[ValidAgeFilter] = filter match {
    case Some(value) => Some(AgeFilterValidator.validate(value)) // ignore
    case None        => Some(None)
  }

  def validateAge2(filter: Option[AgeFilterProto]): Option[ValidAgeFilter] = filter match {
  ...
  def validate(filtersProto: FiltersProto): Option[Filters] = for {
    ...
    ageFilterMaybe <- filtersProto.ageFilter.traverse(AgeFilterValidator.validate)
  } yield Filters(ids, ageFilterMaybe)
}

object GetUsersValidator {

  def validate(req: GetUsersReqProto): Option[GetUsersReq] = ???
}

object Boot3 extends App {
  val ageFilterProto1: Option[AgeFilterProto] = Some(AgeFilterProto(8, FilterModeProto.GT))
  val ageFilterProto2: Option[AgeFilterProto] = Some(AgeFilterProto(12, FilterModeProto.Invalid))
  val ageFilterProto3: Option[AgeFilterProto] = None
  val ageFilterProto4: Option[AgeFilterProto] = Some(AgeFilterProto(12, FilterModeProto.EQ))
